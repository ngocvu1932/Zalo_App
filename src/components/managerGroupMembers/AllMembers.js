import { View, Text, FlatList, Modal, ActivityIndicator, StatusBar, Image } from 'react-native'
import React, { useEffect, useRef, useState } from 'react'
import { Pressable } from 'react-native'
import axios from '../../config/axios'
import { faClose, faKey, faMessage, faPhone } from '@fortawesome/free-solid-svg-icons';
...
export const AllMembers = ({navigation, items}) => {
  ...
  const handleChooseMember = (item) => {
    if (currentId === item.id) {  
      navigation.navigate('Profile', {phoneNumber: item.phoneNumber, isUser: true })
      ...
      <Pressable style={styles.btnChooseMember} onPress={()=>{handleChooseMember(item)}}>
        {item.avatar?.includes('rgb') ? 
          <View style={{height: 55, width: 55, marginLeft: 15}}>
            ...
            {item.id === idAdmin ? 
              <View style={{height: 20, width: 20, borderRadius: 15, backgroundColor: '#848A96', justifyContent: 'center', alignItems: 'center', position: 'absolute', bottom: 0, right: 0}}>
        ...
        <View style={{height: 45, justifyContent: 'space-between', marginLeft: 15}}>
          <Text style={{fontSize: 17}}>{item.id === currentId ? 'Bạn' : item.userName}</Text>
          <Text style={{fontSize: 14, opacity: 0.7}}>{item.id === idAdmin ? 'Trưởng nhóm' : 'Thêm bởi trưởng nhóm'}</Text>
        </View>
      </Pressable>
    )
  }
  ...
  return (
    <View style={{height: "100%"}}>
      <View style={{padding: 10, backgroundColor: '#FFFFFF'}}>
        <Text style={{color: '#008BFA', fontWeight: '400'}}>Thành viên ({members.length})</Text>
      </View>
      ...
      {modalVisible ?
        ...
        >
          <Pressable style={styles.modalContainer} onPress={ ()=> setModalVisible(false)}>
            <View style={[styles.modalContent, heightModal ? {height: 230} : {height: 330}]}>
              <View style={{flexDirection: 'row', justifyContent: 'space-between', height: 55, alignItems: 'center', borderBottomWidth: 1, borderBottomColor: '#DFE0E4'}}>
                <Text style={{color: '#FFFFFF', width: 50}}> X </Text>
                <Text style={{fontSize: 16, fontWeight: '500'}}>Thông tin thành viên</Text>
                <Pressable style={styles.btnPressClose} onPress={()=> setModalVisible(false)}>
              ...
              <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'}}>
                <View style={{height: 70, justifyContent: 'center', alignItems: 'center'}}>
                  ...
                  : 
                    <View style={{height: 55, width: 55, marginLeft: 15}}>
                      <Image source={{uri: idMember.avatar}} style={{height: 55, width: 55, borderRadius: 30}} />
                      {idMember.id === idAdmin ? 
                        <View style={{height: 20, width: 20, borderRadius: 15, backgroundColor: '#848A96', justifyContent: 'center', alignItems: 'center', position: 'absolute', bottom: 0, right: 0}}>
                          <FontAwesomeIcon size={10} color='yellow' icon={faKey} />
                        </View> : 
                ...
                <Text numberOfLines={1} style={{fontSize: 16, flex: 1, marginLeft: 15, fontWeight: '600'}}>{idMember.userName}</Text>
                ...
                <View style={{flexDirection: 'row', marginRight: 15}}>
                  ...
                  <Pressable style={[styles.btnPressOpts]}>
                    <FontAwesomeIcon color='#525459' size={16} icon={faCommentDots} />
                  </Pressable>
                </View>
              </View>

              <Pressable style={styles.btnPressOpts1} onPress={() =>{
                setModalVisible(false); 
                navigation.navigate('Profile', {phoneNumber: idMember.phoneNumber, isUser: false })
                }}>
                <Text style={{fontSize: 15, marginLeft: 15}}>Xem trang cá nhân</Text>
              </Pressable>

              {groupChatInfo.administrator === currentId ? 
                <View>
                  <Pressable style={styles.btnPressOpts1}>
                    <Text style={{fontSize: 15, marginLeft: 15}}>Bổ nhiệm làm phó nhóm</Text>
                  </Pressable>

                  <Pressable style={styles.btnPressOpts1}>
                    <Text style={{fontSize: 15, marginLeft: 15}}>Chặn thành viên</Text>
                  </Pressable>

                  <Pressable style={styles.btnPressOpts1} onPress={() =>{handleRemoveMember()}}>
                    <Text style={{fontSize: 15, marginLeft: 15, color: 'red'}}>Xóa khỏi nhóm</Text>
                  </Pressable>
                </View> 
              : ''}
            </View>
