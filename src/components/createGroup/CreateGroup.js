...
import { Text, View, TextInput, Pressable, Image,  Keyboard, FlatList, StatusBar, ActivityIndicator } from 'react-native'
import { FontAwesomeIcon } from '@fortawesome/react-native-fontawesome';
import { faArrowRight, faCamera, faCheck, faSearch, faXmark } from '@fortawesome/free-solid-svg-icons';
...
export const CreateGroup = ({ navigation }) => {
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            <Pressable style={styles.btnChooseMember} onPress={()=> chooseMember(data?.id, data?.avatar, data?.userName)}>
        ...
        <View style={styles.container}>
            ...
            <View style={[styles.body, modalVisible ? {paddingBottom: 90} : '', device.device === 'ios' ? isKeyboardOpen ? {paddingBottom: keyboardHeight + 90} : '' : '']}>
                <View style={{ flexDirection: 'row', padding: 16, alignItems: 'center' }}>
                    <Pressable style={styles.btnChooseImage} onPress={()=> openImagePickerAsync()}>
                        {isLoadingImage ? 
                        ...
                        : 
                            linkImageGroup ? 
                                <View>
                                    <Image style={{height: 60, width: 60, borderRadius: 30}} source={{uri: linkImageGroup}}></Image> 
                                    <View style={{position: 'absolute',bottom: 0, right: 0, height: 18, width: 18, borderRadius: 10, backgroundColor: '#E5E4E0', justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: '#FFFFFF'}} >
                                        <FontAwesomeIcon size={13} color='#6B849A' icon={faCamera} /> 
                                    </View>
                                </View>
                            : 
                            <FontAwesomeIcon style={{}} color='#898A8C' size={30} icon={faCamera} />
                        }
                    </Pressable>
                    <TextInput style={[styles.textInputNameGroup, isFocus ? {borderBottomWidth: 1, borderBottomColor: '#5DBCD2'} : {}]} placeholderTextColor={'#828387'} placeholder='Đặt tên nhóm' onChangeText={(text)=> setGroupName(text)} onFocus={()=>setIsFocus(true)} onBlur={()=> setIsFocus(false)}></TextInput>
                    {isFocus ? 
                        <Pressable style={{height: 30, width: 30, justifyContent: 'center', alignItems: 'center'}} onPress={()=> Keyboard.dismiss()}>
                            <FontAwesomeIcon color='#029AFD' size={22} icon={faCheck} />
                        </Pressable> : 
                    ''}
                </View>

                <View style={{justifyContent: 'center', flexDirection: 'row', alignItems: 'center', marginBottom: 15}}>
                    ...
                    <View style={{height: '80%',}}> 
                        <View style={{ flexDirection: 'row', height: 80}}>
                            ...
                            <View style={{width: '20%', justifyContent: 'center', alignItems: 'center'}}>
                                <Pressable disabled={isLoadingImage} style={[styles.btnCreateGroup, isLoadingImage ? {backgroundColor: '#C1D4E3'} : {}]} onPress={()=> {createGroup()}}>
